...
class AlertDispatcher:
    ...
    def dispatch(self, decision: RiskDecision) -> DispatchResult:
        ...
        if last_sent is not None and abs(decision.timestamp - last_sent) < self.cooldown:
    ...
    def _restore_state(self) -> None:
        for record in self.recent_alerts():
            ...
            if kind == "fall_event" and record.get("recovery_confirmed") is True:
                self._active_fall_subjects.discard(subject)
                self._last_sent.pop(key, None)
                ...
                if kind == "fall_event":
                    self._active_fall_subjects.add(subject)
