...
参与执行。一次面向最终快照的 smoke run 显示
`camera_health=healthy`、`source_error_count=0`。注意 `decision_count` 仅表示
**最后一次服务快照**中的决策数，不能用于判断全程是否实际进行了模型推理。

随后对同一 30 FPS 本地回放逐帧审计，完整处理了 1280 帧，耗时 57.539 秒（约
22.25 帧/秒）。审计到 1316 个 POSE 事件，其中 308 个为质量合格的模型输出、
871 个为长时域窗口预热、137 个为无人帧；没有运行时异常，也没有生成告警。
这证明真实姿态—时序模型—决策服务链路可以运行。该视频的事件真值没有在本次
审计中与输出对齐，因此“零告警”不能解释为漏报率、误报率或设备性能，只能作为
本地可运行性证据。
