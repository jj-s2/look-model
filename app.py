...
def create_service(*, demo_fixtures: bool) -> LiveMonitoringService:
    ...
    dispatcher = AlertDispatcher(output_directory / "local_alerts.jsonl")
    return LiveMonitoringService(
        sources,
        dispatcher=dispatcher,
        clip_buffer=CircularClipBuffer(output_directory / "event_clips", recording_opt_in=False),
        retention_policy=RetentionPolicy(output_directory / "event_clips", keep_days=7),
    )
